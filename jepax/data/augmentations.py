...
def augment_image(rng, img, color_jitter_prob=0.8, grayscale_prob=0.2, hflip_prob=0.5):
...
@eqx.filter_jit
def augment_batch(
    rng, images, color_jitter_prob=0.8, grayscale_prob=0.2, hflip_prob=0.5
):
    keys = jax.random.split(rng, images.shape[0])

    def aug(k, img):
        return augment_image(k, img, color_jitter_prob, grayscale_prob, hflip_prob)
